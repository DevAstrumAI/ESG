...
export default function Scope1Wizard() {
  ...
  const totalSteps = 5; // 4 categories + summary
  ...
  const steps = [
    { id: 1, name: "Mobile Combustion", icon: "🚗", component: <VehicleTable /> },
    { id: 2, name: "Stationary Combustion", icon: "🏭", component: <StationaryForm /> },
    { id: 3, name: "Refrigerants", icon: "❄️", component: <RefrigerantForm /> },
    { id: 4, name: "Fugitive Emissions", icon: "💨", component: <FugitiveForm /> },
    { id: 5, name: "Summary", icon: "📊", component: <Scope1Summary /> },
  ];
          ...
          {steps.map((step) => (
            <div key={step.id} className="step-wrapper">
              <button
                ...
                className={`step-indicator ${currentStep === step.id ? 'active' : ''} ${hasStepData(step.id) ? 'completed' : ''}`}
                disabled={step.id === 5} // Can't jump to summary directly
              >
